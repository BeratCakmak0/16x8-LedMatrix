...
const uint8_t rows[8] = {B00000001,B00000010,B00000100,B00001000,B00010000,B00100000,B01000000,B10000000};
unsigned long previousMillis;
const long shiftDelay = 50;//define how fast text shift
const int whichMode = 1;//mode 1 = show image mode 2 = scrolling text 
String rowText = "Lorem Ipsum";
...
void ShowImage(){
  static uint8_t regData1,regData2;
  
  for(int i = 0;i < 8;i++){
    regData1 = pgm_read_byte(&image[0][i]);
    regData2 = pgm_read_byte(&image[1][i]);
...
void ScrollingText(){
  ...
  static uint8_t regData1,regData2,regData3;
  unsigned long currentMillis = millis();

  for(int i = 0;i < 8;i++){
    regData1 = pgm_read_byte(&font8x8[rowText.charAt(letterCounter)-32][i]);
    regData2 = pgm_read_byte(&font8x8[rowText.charAt(letterCounter+1)-32][i]);
    regData3 = pgm_read_byte(&font8x8[rowText.charAt(letterCounter+2)-32][i]);
